...
void read_motorized_ASC_indices (FILE *fp, struct m_mc_asc_data *mMcAscIndices)
{
	int i, index, rec_len, n, col, maxIndex;
	int intIndex;
	...
	tempValues = (int**)malloc( Ini->MAX_TAZS * sizeof(int *) );
	tempRow = (int*)malloc( Ini->MAX_TAZS * sizeof(int) );

	temp = (char*)malloc( LRECL * sizeof(char) );

	// skip 2 header record
	...
	while ((fgets(InputRecord, RECORD_LENGTH, fp)) != NULL) {
		...
		while ( i < rec_len ) {
			...
			temp[index] = '\0';
			tempRow[col] = atoi(temp);

			col++;

		}

		tempValues[n] = (int*)malloc( col * sizeof(int) );
		for (i=0; i < col; i++)
	...
	districtData = mMcAscIndices->mMcAscIndices;
	...
	mMcAscIndices->indices = (int**) HeapAlloc (heapHandle, HEAP_ZERO_MEMORY, n*sizeof(int *));
	...
	for (i=0; i < n; i++) {
		...
		mMcAscIndices->indices[i] = (int*) HeapAlloc (heapHandle, HEAP_ZERO_MEMORY, n*sizeof(int));
		for (col=0; col < n; col++)
			mMcAscIndices->indices[i][col] = tempValues[i][col];
	}
