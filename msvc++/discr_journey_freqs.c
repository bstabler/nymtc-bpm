...
//	addRAM ("discr_jrny_freqs", (max_hh+1)*sizeof(int) + 1*10*(next+1)*sizeof(short));

// tally up combined work and school journeys per person
	for (k=0; k < Ini->NUMBER_JOURNEYS; k++) {
		hh_index = (*hh2id)[JourneyAttribs->hh[k]];
		pers_index = 10*hh_index + JourneyAttribs->persno[k];
		if ((JourneyAttribs->purpose[k] >= 0 && JourneyAttribs->purpose[k] <= 2) || JourneyAttribs->purpose[k] == 3)
