...
{
	int i_co, j_co, rec_len;
	char InputRecord[RIVER_RECORD_LENGTH+2];
	char temp[LRECL];


// Read river crossing data records.
	fgets(InputRecord, RIVER_RECORD_LENGTH+2, fp);										// ignore header record
	while ((fgets(InputRecord, RIVER_RECORD_LENGTH+2, fp)) != NULL) {
		InputRecord[RIVER_RECORD_LENGTH] = '\0';
		rec_len = (int)strlen(InputRecord);
		if (rec_len == RIVER_RECORD_LENGTH) {
