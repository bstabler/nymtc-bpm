...
/*
	...
	*  core_names - cor_name[i] = name of core matrix i 
	*  if core_name is null then core names are: "Matrix 1", "Matrix 2", ... , "Matrix N".
	* data_type = numeric data type to store in the matrix (e.g. short_type, long_type, float_type or double_type)
	* RETURNS:
	*  
	* New matrix handle
	*
 ...
 *
 */
MATRIX MATRIX_Copy(MATRIX that, MAT_INFO *Info, short iCore, BOOL CopyAllIndices, long *copy_cores, long nCopyCores);
