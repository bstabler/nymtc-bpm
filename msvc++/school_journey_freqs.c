...
// tally up work journeys per person, school journeys per person, and school journeys per hh
	for (k=0; k < Ini->NUMBER_JOURNEYS; k++) {
...
//	addRAM ("school_jrny_freqs", (max_hh+1)*sizeof(int) + next*sizeof(short) + 2*10*(next+1)*sizeof(short));
}
