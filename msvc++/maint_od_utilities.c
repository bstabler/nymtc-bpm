/*
 *	Maintenance journey mode choice OD related components of systematic utilites for each modal alternative
 *
...
#define MAINT_WT_TRANSFER_WAIT (float)2.0
#define MAINT_WT_TRANSFER_OVTT (float)2.0
#define MAINT_DT_TRANSFER_WAIT (float)2.0
#define MAINT_DT_TRANSFER_OVTT (float)2.0
#define MAINT_WC_TRANSFER_WAIT (float)2.0
#define MAINT_WC_TRANSFER_OVTT (float)2.0
#define MAINT_DC_TRANSFER_WAIT (float)2.0
#define MAINT_DC_TRANSFER_OVTT (float)2.0
	
	
	
void maint_da_od_util (int i, int ncols, float *hwy_time, float *hwy_dist, float *hwy_toll, struct zone_data *ZonalData, int **ranprkcst, float *Utility)
{
	int j;
	...
	for (j=1; j <= ncols; j++) {
		...
		if (hwy_time[j] != MISSING && hwy_dist[j] != MISSING && hwy_toll[j] != MISSING) {
			...
			ParkCost = nw_parking_cost (j, ZonalData, ranprkcst);
...
void maint_sr2_od_util (int i, int ncols, float *hwy_time, float *hwy_dist, float *hwy_toll, struct zone_data *ZonalData, int **ranprkcst, float *Utility)
{
...
void maint_sr3_od_util (int i, int ncols, float *hwy_time, float *hwy_dist, float *hwy_toll, struct zone_data *ZonalData, int **ranprkcst, float *Utility)
{
...
void maint_sr4_od_util (int i, int ncols, float *hwy_time, float *hwy_dist, float *hwy_toll, struct zone_data *ZonalData, int **ranprkcst, float *Utility)
{
...
void maint_wt_od_util (int i, int ncols, float *hwy_time, float *hwy_dist, float *hwy_toll, struct tran_data *TranSkims, struct zone_data *ZonalData, float *Utility)
{
		...
		if (Ivtt > 0.0) {
			if (TranSkims->Xfrs [j] > 0) {
				if (TranSkims->Xfr_wait[j] != MISSING)
					...
					Xfr_wait = TranSkims->Xfrs[j]*MAINT_WT_TRANSFER_WAIT;
//					ExitWithCode (91);
				if (TranSkims->Xfr_ovtt[j] != MISSING)
					Xfr_ovtt = TranSkims->Xfr_ovtt[j];
				else
					Xfr_ovtt = TranSkims->Xfrs[j]*MAINT_WT_TRANSFER_OVTT;
//					ExitWithCode (92);
			}
...
void maint_dt_od_util (int i, int ncols, float *hwy_time, float *hwy_dist, float *hwy_toll, struct tran_data *TranSkims, struct zone_data *ZonalData, float *Utility)
{
		...
		if (Ivtt > 0.0) {
			if (TranSkims->Xfrs [j] > 0) {
				if (TranSkims->Xfr_wait[j] != MISSING)
					...
					Xfr_wait = TranSkims->Xfrs[j]*MAINT_DT_TRANSFER_WAIT;
//					ExitWithCode (93);
				if (TranSkims->Xfr_ovtt[j] != MISSING)
					Xfr_ovtt = TranSkims->Xfr_ovtt[j];
				else
					Xfr_ovtt = TranSkims->Xfrs[j]*MAINT_DT_TRANSFER_OVTT;
//					ExitWithCode (94);
			}
			Ovtt = TranSkims->First_wait[j] + Xfr_ovtt + Xfr_wait;
...
void maint_wc_od_util (int i, int ncols, float *hwy_time, float *hwy_dist, float *hwy_toll, struct tran_data *TranSkims, struct zone_data *ZonalData, float *Utility)
{
		...
		if (Ivtt > 0.0 && TranSkims->CR_ivtt[j] > 0.0) {
			if (TranSkims->Xfrs [j] > 0) {
				if (TranSkims->Xfr_wait[j] != MISSING)
					...
					Xfr_wait = TranSkims->Xfrs[j]*MAINT_WC_TRANSFER_WAIT;
//					ExitWithCode (95);
				if (TranSkims->Xfr_ovtt[j] != MISSING)
					Xfr_ovtt = TranSkims->Xfr_ovtt[j];
				else
					Xfr_ovtt = TranSkims->Xfrs[j]*MAINT_WC_TRANSFER_OVTT;
//					ExitWithCode (96);
			}
			Ovtt = TranSkims->First_wait[j] + Xfr_ovtt + Xfr_wait;
			Cost = TranSkims->Fare[j]*(float)100.0;									// cost in cents
...
void maint_dc_od_util (int i, int ncols, float *hwy_time, float *hwy_dist, float *hwy_toll, struct tran_data *TranSkims, struct zone_data *ZonalData, float *Utility)
{
		...
		if (Ivtt > 0.0 && TranSkims->CR_ivtt[j] > 0.0) {
			if (TranSkims->Xfrs [j] > 0) {
				if (TranSkims->Xfr_wait[j] != MISSING)
					...
					Xfr_wait = TranSkims->Xfrs[j]*MAINT_DC_TRANSFER_WAIT;
//					ExitWithCode (97);
				if (TranSkims->Xfr_ovtt[j] != MISSING)
					Xfr_ovtt = TranSkims->Xfr_ovtt[j];
				else
					Xfr_ovtt = TranSkims->Xfrs[j]*MAINT_DC_TRANSFER_OVTT;
//					ExitWithCode (98);
			}
			Ovtt = TranSkims->First_wait[j] + Xfr_ovtt + Xfr_wait;
			DrAcc_ovtt = TranSkims->Acc_ovtt[j];
...
void maint_tx_od_util (int i, int ncols, float *hwy_time, float *hwy_dist, float *hwy_toll, struct zone_data *ZonalData, struct taxi_data *TaxiData, float *Utility)
{
	int j;
