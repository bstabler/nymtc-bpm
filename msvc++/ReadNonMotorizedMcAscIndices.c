...
void read_non_motorized_ASC_indices (FILE *fp, struct nm_mc_asc_data *nmMcAscIndices)
{
	...
	while ((fgets(InputRecord, RECORD_LENGTH, fp)) != NULL) {
		...
		temp[index] = '\0';
		tempValues[n] = atoi(temp);

		n++;
	}

	districtData = nmMcAscIndices->nmMcAscIndices;
	...
	nmMcAscIndices->indices = (int*) HeapAlloc (heapHandle, HEAP_ZERO_MEMORY, n*sizeof(int));
	...
	for (i=0; i < n; i++) {
		...
		nmMcAscIndices->indices[i] = tempValues[i];
	}
