...
{
	int taz, rec_len;
	char InputRecord[ZONE_RECORD_LENGTH+2];
	char temp[LRECL];

	

// Read zonal data records.
	fgets(InputRecord, ZONE_RECORD_LENGTH+2, fp);								// ignore header record
	while ((fgets(InputRecord, ZONE_RECORD_LENGTH+2, fp)) != NULL) {
		InputRecord[ZONE_RECORD_LENGTH] = '\0';
		rec_len = (int)strlen(InputRecord);
		if (rec_len == ZONE_RECORD_LENGTH) {
