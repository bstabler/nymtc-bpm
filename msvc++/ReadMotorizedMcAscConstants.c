...
void read_motorized_ASC_constants (FILE *fp, struct m_mc_asc_data *mMcAscData)
{
	...
	districtData = mMcAscData->mMcAscConstants;
	...
	mMcAscData->constants = (float**) HeapAlloc (heapHandle, HEAP_ZERO_MEMORY, n*sizeof(float *));
	...
	for (i=0; i < n; i++) {
		...
		mMcAscData->constants[i] = (float*) HeapAlloc (heapHandle, HEAP_ZERO_MEMORY, col*sizeof(float));
		for (j=0; j < col; j++)
			mMcAscData->constants[i][j] = tempValues[i][j];
	}
