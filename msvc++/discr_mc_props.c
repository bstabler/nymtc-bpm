...
void discr_mc_props (int journey, int *Avail, int person_type, int autos, int *walk, int PrevAuto, float *OD_Utility, float *SE_Utility, float *Prob, float *ASC, float *TotalUtility)
{
...
// walk access available at origin, and walk egress available at destination.
	...
	for (i=0; i < Ini->NUMBER_ALTS; i++)		// no non-motorized or school bus available for discretionary
		Avail[i] = 1;
	Avail[9] = 0;
	Avail[10] = 0;
